...
public class GameServer implements Runnable, Constants{
    ...
    /**
     * The chat module responsible for TCP connections for chat
     */
    ChatServer chat;
    ...
    public GameServer(int numPlayers){
        ...
        try {
            serverUDPSocket = new DatagramSocket(UDP_PORT);
            serverUDPSocket.setSoTimeout(100);
        } catch (IOException e) {
            System.err.println("Could not listen on port: "+UDP_PORT);
            System.exit(-1);
        }catch(Exception e){}
            ...
            switch(gameStage){
                ...
                case IN_PROGRESS:
                    ...
                    if (playerData.startsWith("PLAYER")){
                        //Tokenize:
                        //The format: PLAYER <player name> <x> <y> <puckValue>
                        String[] playerInfo = playerData.split(" ");
                        String pname =playerInfo[1];
                        int x = Integer.parseInt(playerInfo[2].trim());
                        int y = Integer.parseInt(playerInfo[3].trim());
                        int puckVal = Integer.parseInt(playerInfo[4].trim());
                        //Get the player from the game state
                        NetPlayer player=(NetPlayer)game.getPlayers().get(pname);
                        player.setX(x);
                        player.setY(y);
                        player.setPuckValue(puckVal);
